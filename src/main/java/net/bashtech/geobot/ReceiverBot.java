...
public class ReceiverBot extends PircBot {
	...
	private Pattern vinePattern = Pattern.compile(
			".*(vine|4).*(4|vine).*Google.*", Pattern.CASE_INSENSITIVE);
	...
	protected void onChannelMessage(String channel, String sender,
		...
		if (channelInfo.isRegular(sender)
				|| (channelInfo.subscriberRegulars && channelInfo
						.isSubscriber(sender)))
			isRegular = true;
		...
		if (!isOp && channelInfo.config.getBoolean("globalFilter")
				&& this.isGlobalBannedWord(message)) {
			// this.secondaryBan(channel, sender, FilterType.GLOBALBAN);
			this.secondaryTO(channel, sender, 86400, FilterType.GLOBALBAN,
					message);
		...
		if (channelInfo.useFilters) {
			...
			if (!isRegular) {
				String normalMessage = org.apache.commons.lang3.StringUtils
						.stripAccents(message);
			...
			if (channelInfo.getFilterCaps() && !isRegular) {
				...
				double capsPercent = ((double) capsNumber / messageNoWS
						.length()) * 100;
				if (channelInfo.getFilterCaps()
						&& !(isRegular)
						&& message.length() >= channelInfo
		...
		// Old Style !newtweet
		if ((tweetServer.length() > 0)
				&& msg[0].equalsIgnoreCase(prefix + "newtweet") && isOp) {
			log("RB: Matched command !newtweet");
			channelInfo.config.setString("siteTweet", fuseArray(msg, 1));
			send(channel,
					"Updated http://" + tweetServer + "/"
							+ channel.substring(1) + "/");
		}
		...
		if ((msg[0].equalsIgnoreCase(prefix + "viewers") || msg[0]
			...
			if (BotManager.getInstance().twitchChannels) {
				try {
					send(channel, JSONUtil.krakenViewers(twitchName)
							+ " viewers.");
				} catch (Exception e) {
		...
		if (msg[0].equalsIgnoreCase(prefix + "bitrate")) {
			...
			if (bitrate > 1)
				send(channel, "The source bitrate is " + (int) bitrate
						+ " Kbps");
			else
		...
		if (msg[0].equalsIgnoreCase(prefix + "uptime")) {
			...
			try {
				...
				send(channel, "Streaming for " + this.getTimeStreaming(uptime)
						+ " since " + uptime + " PST.");
			} catch (Exception e) {
		...
		if (msg[0].equalsIgnoreCase(prefix + "music")
				|| msg[0].equalsIgnoreCase(prefix + "lastfm")) {
		...
		// !throw - All
		if (msg[0].equalsIgnoreCase(prefix + "throw")
				&& (channelInfo.checkThrow() || isRegular)) {
		...
		if (msg[0].equalsIgnoreCase(prefix + "commercial")
			...
			if (isOwner) {
				...
				// send(channel,
				// "Running a 30 second commercial. Thank you for supporting the channel!");
			}
			return;
		}
		...
		if (msg[0].equalsIgnoreCase(prefix + "skipcommercial")
			...
			if (isOwner) {
				...
				send(channel,
						"The next commercial command will be ignored. If you have the command overridden with a custom command it will still be displayed.");
			}
			return;
		}
		...
		if (msg[0].equalsIgnoreCase(prefix + "poll") && isOp) {
			...
			if (msg.length < 2) {
				send(channel,
						"Syntax: \"!poll create [option option ... option]\"");
			} else if (msg.length >= 2) {
				if (msg[1].equalsIgnoreCase("create")) {
					...
					send(channel,
							"Poll created. Do '!poll start' to start voting.");
					...
					if (channelInfo.getPoll() != null) {
						if (channelInfo.getPoll().getStatus()) {
							...
							send(channel,
									"Poll started. Type: !vote <option> to start voting.");
						}
					}
		...
		if (msg[0].equalsIgnoreCase(prefix + "filter") && isOwner) {
			...
			if (msg[0].equalsIgnoreCase("status")) {
				...
				send(channel,
						"Banned phrases: "
								+ channelInfo.getFilterOffensive()
								+ " ~ severity="
								+ channelInfo.config
										.getInt("banPhraseSeverity"));
				send(channel,
				...
				if (msg.length == 1) {
					...
					if (msg[1].equalsIgnoreCase("on")) {
						...
						if (msg.length > 2 && Main.isInteger(msg[2])) {
							...
							channelInfo.config.setInt("banPhraseSeverity",
									severity);
			...
			if (msg.length == 1) {
				...
				if (msg.length < 3) {
					send(channel,
							"Subscriber alerts: "
									+ channelInfo.config
											.getBoolean("subscriberAlert"));
					send(channel, "Subscriber alert message: "
							+ channelInfo.config.getString("subMessage"));
					...
					channelInfo.config.setString("subMessage",
							fuseArray(msg, 3));
		...
		if (this.getNick().equalsIgnoreCase(sender)) {
			synchronized (joinedChannels) {
				...
				if (!joinedChannels.contains(channel)) {
					...
					System.out.println("DEBUG: Got JOIN for "
							+ joinedChannels.size() + " "
							+ this.getChannels().length);
	...
	public void send(String target, String sender, String message, String[] args) {
		...
		message = MessageReplaceParser.parseMessage(target, sender, message,
				args);
